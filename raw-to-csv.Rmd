---
title: 'Group Homework: Data Preparation'
author: "Wranglers"
...
The data for patent bibliographic data exists in 533 zipped files. The format for the relevant data is pseudo-xml. The following process is used to extract the cleaned data:  

### 1. Unzip and Transform Raw `ipgbXXXXXXXX_wkXX.xml` Files  

Each `*.zip` file is extracted, and the `.xml` transformed, so that each patent is on a single line. These patent per line files are filtered, so that only patents belonging to our company and its competitors are kept. Shell code:   
```{r eval=FALSE}
...
### 2. Prepare Each Company Patents for Feature Extraction  

The one patent per line text file is split into 4 files: one for the patents belonging to our company, and three for each of our closest competitors:  
```{r eval=FALSE}
...
# load mined .csv
setwd('~/DSBA-6100/group project/patent_csvs/')
...
# Visualize Abstracts and Titles for Medtronic
